...
import { ArrowUpRight, ArrowRight, ArrowDown, ChevronDown, Zap, BarChart3, Target, Code2, Eye, Sparkles, Search, Layers, CheckCircle2, Compass, Cpu, Activity, User, type LucideIcon } from 'lucide-react'
    ...
    >
      <div className="flex flex-col gap-1 md:flex-row md:items-baseline md:justify-between md:gap-4">
        ...
        <span
          className={
            isBlue
              ? 'type-sans-bold w-fit rounded px-1.5 py-0.5 text-[9px] leading-normal text-white max-md:!font-[700] md:rounded-none md:px-0 md:py-0 md:text-micro md:!font-[650]'
...
function PhaseCardTile({ card, fixedSize }: { card: PhaseCard; fixedSize: boolean }) {
  ...
  return (
    <div
      className={`flex flex-col gap-4 rounded-[20px] p-5 min-[900px]:px-5 min-[900px]:py-6 ${
    ...
    >
      ...
      <div className="flex flex-col gap-1.5 min-[900px]:block">
        ...
        <p className="type-sans-regular text-[13px] leading-[18px] text-white/90 min-[900px]:mt-1.5 min-[900px]:text-eyebrow min-[900px]:leading-normal">
          {card.description}
        </p>
      </div>
    </div>
...
function PhasesRow({ cards, startIndex }: { cards: PhaseCard[]; startIndex: number }) {
  return (
...
// iPad 900–1200: two rows of three, EXPLORE FAST under the first,
// DESIGN DEEP under the second, one down-arrow between the rows.
function PhasesGridIpad({ section }: { section: ProcessAndAi['phasesSection'] }) {
  ...
  return (
    <div className="hidden min-[900px]:max-[1199.98px]:block">
      <PhasesRow cards={row1} startIndex={0} />
      <div className="mt-6">
      ...
      <div className="mt-6">
        <PhasesLabel label={section.footerNote.endLabel} tone="end" align="end" />
      </div>
    </div>
  )
}
...
export function DualPrototypeFramework({ framework }: { framework: ProcessAndAi['frameworkSection'] }) {
  ...
  return (
    <section className="section-shell flex flex-col gap-10 border-t-0 bg-[#FFFCF7] py-16 [--section-pad-x:1rem] md:block md:border-t md:border-neutral-200/60 md:py-20 md:[--section-pad-x:1.5rem]">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
...
export function MethodologyAccordion({ data = DEFAULT_DATA }: { data?: FrameworkSectionData }) {
  return (
    <section className="border-t-0 bg-[#FFFCF7]">
      <div className="section-shell py-16 [--section-pad-x:1rem] md:py-[105px] md:[--section-pad-x:1.5rem]">
        <FrameworkHeader
          badge={data.badge}
...
export function AiSynergyGrid({ synergy }: { synergy: ProcessAndAi['aiAcceleratesSection'] }) {
  return (
    <section className="bg-white">
    <div className="section-shell py-16 [--section-pad-x:1rem] md:py-20 md:[--section-pad-x:1.5rem]">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
...
export function OutcomesGrid({ outcomes }: { outcomes: ProcessAndAi['outcomesSection'] }) {
  return (
    <section className="bg-[#FFFCF7] py-20 md:py-[120px]">
        ...
        {outcomes.grid.map((item, idx) => {
          ...
          return (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: idx * 0.05 }}
              className="flex min-h-0 flex-col rounded-2xl border border-[#DEDFE0] bg-white p-8 md:min-h-[245px] md:rounded-[24px]"
            >
              <div>
                {IconComponent && (
                  <div className="mb-6 flex h-10 w-10 items-center justify-center rounded-full bg-neutral-100 text-[#2538F5] md:h-12 md:w-12">
