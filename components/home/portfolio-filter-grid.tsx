...
export function PortfolioFilterGrid({ content }: { content: HomePage["portfolio"] }) {
  ...
  const clear = () => router.replace("#work", { scroll: false })
  return <section id="work" className="bg-background py-20 md:py-28"><div className="mx-auto max-w-7xl px-5 md:px-6">
    ...
    <div className="mt-8 flex flex-wrap gap-3">{content.filters.map((filter,i)=><button key={filter} onClick={i===0?clear:undefined} className={`flex items-center gap-2 rounded-full border px-5 py-2 text-sm ${i===0 && !sector && !discipline ? "border-primary bg-primary text-primary-foreground":"border-border"}`}>{filter}{i>0&&<ChevronDown size={13}/>}</button>)}</div>
